...
function validateImport(csvText: string): { rows: ImportRow[]; headerError?: string } {
  ...
  const rows: ImportRow[] = raw.slice(1).map((r, idx) => {
    ...
    // Imported projects obey the same admin-configured requirement rules as the
    // create form. Deduped by message: AP/assignee/MCP checks above already
    // produce the identical "<label> is required" strings.
    const reqRules = db.prepare(
      `SELECT field_name, label FROM field_requirements
       WHERE object_type = 'project' AND required = 1 AND required_at IN ('creation', 'always')`
    ...
    const provided: Record<string, unknown> = {
      ...
      // fall back to the raw cell so a provided-but-invalid value (already
      // flagged above) doesn't add a second, misleading "is required" error
