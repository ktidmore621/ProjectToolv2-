/**
 * Seed logic for default configuration (picklists, default workflow template,
 * field requirement rules, view layouts, RAG thresholds) and demo data.
 * The `npm run seed` CLI lives in seed.ts; tests import from here.
 */
...
export function resetData() {
  db.exec(`
    ...
    DELETE FROM counters; DELETE FROM sqlite_sequence;
  `);
  db.prepare("INSERT INTO counters (key, value) VALUES ('project_code', 0)").run();
}

export function isSeeded(): boolean {
  return (db.prepare("SELECT COUNT(*) AS n FROM picklists").get() as { n: number }).n > 0;
}

/** Seed everything. Pass { demo: false } for configuration only (used by tests). */
export function seed(opts: { demo?: boolean } = {}) {
  ...
  // ---- Demo data ----
  if (opts.demo !== false) seedDemo(users, tplId);
}
